...
| Protocol | **Recommended `2026-07-28`:** `server/discover` + per-request `_meta`. `initialize` (`2025-06-18`) is a compatibility pathway only — first reply includes the runtime-upgrade manual. Unsupported versions return JSON-RPC `-32022`. Main-surface resources (`nbcad://document`, `scene`, `sketch`, `profiles`, `features`, `drawing`, `assembly`, `assembly_solution`, `guidance`, `workspace`, …) and prompts (`model_box`, `model_solid`, `model_print_tool`, `model_print_kit`, `tutor_exam`, `import_step`, `drawing_export`, `assemble_joint`, `check_interference`, `undo_history`, `invoke`, …) are advertised. |
| Tools | **206** modeling tools + 9 print helpers + 13 control tools (includes 55 drawing-pack tools and 38 named `assembly_*` tools). New process starts in sketch with solid soft. Spine always has `cad_agent_guidance`, `solid_check`, assembly entry/motion/interference, and `cad_drawing_create_sheet`. Mechanical escape hatches: `cad_invoke`, `cad_drawing_command`. |
...
Spine → active pack → soft packs (60 s TTL, LRU 3). Hidden tools stay
