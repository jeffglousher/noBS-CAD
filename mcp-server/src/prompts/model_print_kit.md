...
Printer: Bambu Lab X2D (256×256×260, 8 mm margin). `spec.scale` shrinks the X2D-max source (exam default 0.4). Clamp rollers ≥ Ø8 / thin h3.2, TE ≥ 0.8, walls ≥ 4 nozzles. The bearing is a **thin flat thrust** under the plate (large PCD) — not a washer stack and not a tall drum.
...
No FDM press fits. Slicer XY hole compensation stays 0. Thrust is a flat land with 0.20 mm float at **every** running land (flange↔rollers, rollers↔plate, plate↔retainer). The machine's job is **rotation about Z**. A **thin, wide** printed thrust pack under the plate provides that. Overturning from tall blades is a couple across a large pitch circle — not a tall journal you can see from the side. Cage height **matches** roller height. The retainer is a washer above the plate (OD between plate bore and flange OD), not a cap through the plate. Every bed-printed functional hole gets a 2-nozzle elephant-foot lead-in. Do **not** print the plate around the PIP rollers on the bed.
...
- PIP at assembled running clearance (cage pockets or rollers under the plate at +0.40). Same-plate gap must be **2 nozzles**
- A friction or running bore on the print bed with no lead-in (elephant foot closes +0.16 and pinches a race)
- Nesting the plate around the PIP rollers on the bed (0.10 mm/side — they fuse)
...
| Roller cartridge | Cage + ≥6 thin PIP rollers, min Ø8 / h3.2, height < diameter, large PCD **under the plate**. Cage height = roller height | Flat, PIP on the kit plate |
...
The bearing is a **printed thrust cartridge** (short PIP rollers in a cage) on a **large pitch circle under the plate**. That is a normal 2026 printed turntable: width takes the moment; height stays a puck. No metal 608, no 623, no tall two-land sleeve, no orange tower.

- Lower race: axle **flange** (printed separately)
- Rolling elements: ≥6 standing pucks, height < diameter, floor h3.2, PIP in the cage
...
- Do not PIP the rollers under the plate. Do not close a top inward lip over the rollers
...
4. Rotor: **root plate** (print sit + upper thrust race, ≥5 mm). Loft three helical **NACA 0021** from the plate top (sit plane), open drafted tips. Do not start the loft from a surface above the plate. Do not grow a tall drum or moment webs on the plate
5. Roller cage + thin PIP rollers on a large PCD **under the plate**. Cage height = roller height. Retainer washer above the plate
